...
export default function AdminPanel() {
  ...
  async function handleAddEvent(e) {
    ...
    setEventType(""); setShowForm(false);
  ...
  function handleDayClick(dateStr, status) {
    ...
    setSelectedDate(dateStr); setEventType("");
    setForm({ couple:"", venue:"", time:"", notes:"", addon:"" });
  ...
  const BgDecor = () => (
    <>
      ...
      <div style={{ position:"fixed", inset:0, zIndex:0, pointerEvents:"none", backgroundImage:`radial-gradient(circle, rgba(30,96,213,0.1) 1px, transparent 1px)`, backgroundSize:"32px 32px", maskImage:"radial-gradient(ellipse at center, black 0%, transparent 75%)", WebkitMaskImage:"radial-gradient(ellipse at center, black 0%, transparent 75%)" }} />
      <div style={{ position:"fixed", top:0, right:0, width:200, height:200, pointerEvents:"none", zIndex:0, opacity:0.12, transform:"rotate(90deg)" }}>
        <svg width="200" height="200" viewBox="0 0 200 200" fill="none">
          <path d="M0 80 L80 0" stroke="#1e60d5" strokeWidth="1"/>
          <path d="M0 120 L120 0" stroke="#1e60d5" strokeWidth="0.5"/>
        </svg>
      </div>
      <div style={{ position:"fixed", bottom:0, left:0, width:200, height:200, pointerEvents:"none", zIndex:0, opacity:0.12, transform:"rotate(270deg)" }}>
        <svg width="200" height="200" viewBox="0 0 200 200" fill="none">
          <path d="M0 80 L80 0" stroke="#1e60d5" strokeWidth="1"/>
          <path d="M0 120 L120 0" stroke="#1e60d5" strokeWidth="0.5"/>
        </svg>
  ...
  if (!isLoggedIn) return (
    <>
      ...
      <div style={{ minHeight:"100vh", background:"linear-gradient(135deg, var(--navy) 0%, var(--navy-mid) 50%, var(--blue-1) 100%)", display:"flex", alignItems:"center", justifyContent:"center", padding:20, position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ position:"absolute", top:-120, right:-80, width:450, height:450, borderRadius:"50%", background:"rgba(64,128,240,0.15)", filter:"blur(60px)", pointerEvents:"none" }} />
        <div style={{ position:"absolute", bottom:-80, left:-60, width:350, height:350, borderRadius:"50%", background:"rgba(21,53,160,0.2)", filter:"blur(50px)", pointerEvents:"none" }} />
        {/* rings */}
        <div style={{ position:"absolute", top:"10%", left:"5%", width:300, height:300, borderRadius:"50%", border:"1px solid rgba(255,255,255,0.05)", pointerEvents:"none" }} />
        <div style={{ position:"absolute", bottom:"15%", right:"8%", width:200, height:200, borderRadius:"50%", border:"1px solid rgba(255,255,255,0.06)", pointerEvents:"none" }} />
    ...
    <>
      ...
      <div style={{ minHeight:"100vh", position:"relative", overflow:"hidden" }}>
        ...
        <main style={{ maxWidth:1080, margin:"0 auto", padding:"32px 20px", position:"relative", zIndex:1 }}>
          {success && (
            <div className="scale-in" style={{ background:"rgba(240,253,244,0.95)", border:"1px solid #86efac", color:"#15803d", padding:"12px 20px", borderRadius:14, marginBottom:24, fontSize:13, fontWeight:600, backdropFilter:"blur(8px)" }}>
              ✅ {success}
            </div>
          )}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 380px", gap:24, alignItems:"start" }}>
            ...
            <div className="card" style={{ overflow:"hidden", boxShadow:"var(--shadow)" }}>
              <div style={{ background:"linear-gradient(135deg, var(--navy), var(--blue-1))", padding:"20px 28px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"relative", overflow:"hidden" }}>
                ...
                <button onClick={() => setCurrentDate(new Date(year,month-1,1))}
                  style={{ background:"rgba(255,255,255,0.1)", border:"1.5px solid rgba(255,255,255,0.2)", color:"#fff", width:36, height:36, borderRadius:10, fontSize:16, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", position:"relative", zIndex:1 }}>‹</button>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
                ...
                <div className="card slide-right" style={{ padding:24, borderTop:"3px solid var(--blue-2)", boxShadow:"var(--shadow)" }}>
                  ...
                  <p style={{ fontSize:12, color:"var(--muted)", marginBottom:16, fontWeight:500 }}>
                    ...
                    {!isWeekend(selectedDate) && (
                      <span style={{ marginLeft:8, background:"rgba(239,68,68,0.1)", color:"#ef4444", fontSize:10, padding:"2px 8px", borderRadius:10, fontWeight:700 }}>
                        ⚠️ Bersyarat
                      </span>
                    )}
                  </p>
                      ...
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                        {[
                          ...
                          <button key={type} type="button" onClick={() => setEventType(type)}
                            ...
                            onMouseEnter={e => { e.currentTarget.style.borderColor="var(--blue-2)"; e.currentTarget.style.background="#eef4ff"; e.currentTarget.style.transform="translateY(-2px)"; }}
                            onMouseLeave={e => { e.currentTarget.style.borderColor="var(--border)"; e.currentTarget.style.background="rgba(248,250,255,0.8)"; e.currentTarget.style.transform="translateY(0)"; }}
                          >
                  ...
                  {eventType && (
                    <>
                      ...
                      {formError && (
                        <div style={{ background:"rgba(255,245,245,0.9)", color:"#dc2626", padding:"8px 12px", fontSize:12, borderRadius:10, marginBottom:14, border:"1px solid #fecaca", fontWeight:500 }}>
                          ⚠️ {formError}
                        </div>
                      )}
              ...
              <div className="card" style={{ overflow:"hidden", boxShadow:"var(--shadow-sm)" }}>
                <div style={{ padding:"16px 20px", borderBottom:"1px solid var(--border)", background:"rgba(232,238,247,0.8)", display:"flex", justifyContent:"space-between", alignItems:"center", backdropFilter:"blur(4px)" }}>
