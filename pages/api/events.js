...
export default async function handler(req, res) {
  ...
  // POST
  if (req.method === "POST") {
    const { couple, venue, time, notes, addon, date, date_end, event_type } = req.body;
    ...
    const { data, error } = await supabase
      .from("wedding_events")
      .insert([{ couple, venue, time, notes, addon, date, date_end: finalDateEnd, event_type, created_at }])
      .select()
      .single();
    if (error) return res.status(500).json({ error: error.message });
